...
// by Vaishnavi Deshpande (Back End Dev 2), and also a few changes regarding the back end
// functionality
...
public class UserInterface {
   ...
   * 
   * @param barcode
   * @return product generated with the attributes specified by the user
   */
   ...
   * user inputs 'R' or 'r'. Displays the attributes of the product that has to be removed.
   * 
   * @param productMap
   */
  public static void removeProduct(HashTableMap<Long, Product> productMap) {
    ...
    try {
      ...
      System.out
          .println("Failed attempt of removing the product which does not exist in the system!");
      return;
    }
   ...
   * 
   * It is done by parsing through the file and then sorting the input, and if it is correct it gets
  ...
  /**
   * Partially matches products stored in the inventory to the first and last three values stored in
   * its barcode (key) value. Prints out the set of all products with common first and last three
   * barcode digits.
   * 
   * @param productMap
   */
  ...
  public static void runProgram(HashTableMap<Long, Product> productMap) {
    ...
    do {
      // stores the first character of the input entered by the user
      userIn = scnr.nextLine();
      System.out.println(userIn);
      // checks the user input and calls the appropriate method based on the instruction
      if (userIn.compareTo("A") == 0 || userIn.compareTo("a") == 0 ) {
        userPromptAdd(productMap);
      } else if (userIn.compareTo("L") == 0  || userIn.compareTo("l") == 0 ) {
        userPromptLoadFile(productMap);
      } else if (userIn.compareTo("R") == 0  || userIn.compareTo("r") == 0 ) {
        removeProduct(productMap);
      } else if (userIn.compareTo("F") == 0  || userIn.compareTo("F") == 0 ) {
        findProduct(productMap);
      } else if (userIn.compareTo("M") == 0  || userIn.compareTo("m") == 0 ) {
